...
class Game:
    ...
    def load_from(path: str) -> "Game":
        ...
        game.rounds_price = {
            kinds[kind_name]: [Price(price) for price in prices]
            ...
            kinds[kind_name]: [
                (
                    Company(
                        ...
                        prices=[Price(price) for price in company["prices"]],
                    )
    ...
    def buy_company(
        self, kind: CompanyKind, name: str, players_share: dict[Player, int], price: Price
    ) -> None:
